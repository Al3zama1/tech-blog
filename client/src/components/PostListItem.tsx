  ...
  return (
    <div className='flex flex-col xl:flex-row xl:gap-5 gap-2 mb-12'>
        <div className='xl:w-1/3'>
            <Image src={post.coverImg} className='rounded-2xl object-cover ' w='535'   />
        </div>
        <div className='flex flex-col xl:w-2/3'>
            ...
            <div className='flex items-center gap-2 text-gray-400 text-sm'>
                ...
                <AuthorHover author={post.author}>
                  <Link to={`/authors/${post.author.id}`} className='text-blue-800'>{`${post.author.firstName} ${post.author.lastName}`}</Link>
                </AuthorHover>
                <span>on</span>
